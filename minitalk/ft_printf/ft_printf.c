...
int	ft_process_str(const char *str, va_list args, int count, int *i)
{
	...
	{
		if (str[*i] == '%' && str[*i + 1] == '%')
			count += ft_printf_putchar_fd('%', 1, i);
		else if (str[*i] == '%' && str[*i + 1] == 'c')
			count += ft_printf_putchar_fd(va_arg(args, int), 1, i);
		else if (str[*i] == '%' && str[*i + 1] == 's')
			count += ft_printf_putstr_fd(va_arg(args, char *), 1, i);
		else if (str[*i] == '%' && (str[*i + 1] == 'd' || str[*i + 1] == 'i'))
			count += ft_printf_putnbr_fd(va_arg(args, int), 1, i);
			...
			count += ft_printf_putchar_fd(str[*i], 1, NULL);
		*i += 1;
	}
	return (count);
}
